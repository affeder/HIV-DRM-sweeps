...
glmm.means <- apply(resamp.dat.glmm.1995.lte4, 2, mean, na.rm = TRUE) #+ fe.coef.all.lte4
...
pdf("../figures/F3-S3-1995-trunc.pdf", width =6, height =5)
par(mar = c(4,4,1, 1))
plot(percentfail, rand.effs[matched.effects], xlab = "Percentage of patients with virologic suppression after 48 weeks" , ylab =  expression(paste("Change in diversity accompanying each DRM (", Delta, "DRM)", sep = "")), type = "n", xlim = c(0, 105), ylim = c(-2, 2))
